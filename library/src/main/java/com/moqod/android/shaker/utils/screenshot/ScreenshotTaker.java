...
public final class ScreenshotTaker {
     ...
     *                                        screenshot
     */
    public static Bitmap getScreenshotBitmap(Activity activity, @Nullable View[] ignoredViews) {
        if (activity == null) {
...
//        Logger.d("viewRoots count: " + viewRoots.size());
    ...
    private static void drawRootToBitmap(final RootViewInfo rootViewInfo, Bitmap bitmap,
        ...
        // support dim screen
        if ((rootViewInfo.layoutParams.flags & WindowManager.LayoutParams.FLAG_DIM_BEHIND)
                == WindowManager.LayoutParams.FLAG_DIM_BEHIND) {
            Canvas dimCanvas = new Canvas(bitmap);

            int alpha = (int) (255 * rootViewInfo.layoutParams.dimAmount);
            dimCanvas.drawARGB(alpha, 0, 0, 0);
        }

        final Canvas canvas = new Canvas(bitmap);
        canvas.translate(rootViewInfo.left, rootViewInfo.top);
